...
public class GCScheme implements GConstants {
	...
	public static GCScheme getColor(PApplet theApplet, int schemeNo){
		app = theApplet;
		if(image == null){
			InputStream is = app.createInput("gui/Icons/vmd/data/user_col_schema.png");
			if(is != null){
				try {
					is.close();
				} catch (IOException e) {
				...
				image = app.loadImage("gui/Icons/vmd/data/user_col_schema.png");
				GMessenger.message(USER_COL_SCHEME, null, null);
			}
			else {
				// User image not provided
				image = app.loadImage("gui/Icons/vmd/data/default_col_schema.png");
			}
		}
